...
def report_format(collected_nodes):
    ...
    for node in collected_nodes:
        ...
        data.append([name, input_shape, output_shape, parameter_quantity,
                     inference_memory, MAdd, duration, Flops, Bops, mread,
                     mwrite])
    df = pd.DataFrame(data)
    df.columns = ['module name', 'input shape', 'output shape',
                  'params', 'memory(MB)',
                  'MAdd', 'duration', 'Flops', 'Bops', 'MemRead(B)', 'MemWrite(B)']
    ...
    total_df = pd.Series([total_parameters_quantity, total_memory,
                          total_operation_quantity, total_flops,total_bops,
                          total_duration, mread, mwrite, total_memrw],
                         index=['params', 'memory(MB)', 'MAdd', 'Flops', 'Bops', 'duration[%]',
    ...
    df['Bops'] = df['Bops'].apply(lambda x: '{:,}'.format(x))
    ...
    summary += "Total Bops: {}Bops\n".format(round_value(total_bops))
